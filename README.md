...
- В ambilight яркость подсветки следует за реальной яркостью экрана — тёмная сцена даёт
  тусклый свет, светлая — яркий (плюс ползунок "Яркость" сверху как общий потолок).
- Тесты: `pytest`.
